...
prod_dir="$(find . -maxdepth 1  -type d -name "production_*")"
...
echo "Remove cache from '../../web/cache/'.."

rm -f ../../web/cache/*.js > /dev/null
rm -f ../../web/cache/*.css
rm -f ../../web/cache/*.txt
x_re
...
if [[ "$( echo ${prod_dir} | wc -c )" -ge "2" ]]; then
  echo "Start moving HTTP-caches -> prod"
...
if [[ "$( echo ${dev_dir} | wc -c )" -ge "2" ]]; then
  echo "Start moving HTTP-caches -> dev"
...
php ../../bin/console sw:generate:attributes > /dev/null ; x_re
